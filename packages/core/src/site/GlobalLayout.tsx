...
export const GlobalLayout = defineComponent<GlobalFrameProps>( {
    name: 'Frame',
    /*
    init: () => null,
    actions: {
        ...
        <>
            <nav className='navigation'>
                <section className='container'>
                    <h2 className='title'>Few</h2>
                    <ul className='frame-type'>
                        {
                            items && Object.entries( items ).map( ( [ key, cb ] ) =>
                                <li key={key}>
                                    <a href='#' onClick={( e ): void => { e.preventDefault(); cb(); }}>{key}</a>
                                </li>
                             )
                        }
                    </ul>
                    <a href='https://github.com/jesse23/few' rel='noopener' title='Few on Github'>
